...
def minimax(board, depth, max_player, alpha=float('-inf'), beta=float('inf'), horizon_risk=0.0,
    ...
    if max_player:
        ...
        for move_tuple in ordered_moves:
            ...
            n_depth, nhr, ndiff = adjust_depth(board, move_tuple, depth, real_depth)

            """ Chess  move """
            board.push(move_tuple[0])
            if real_depth == 0 and opportunities > 1 and board.is_repetition(2):
                board.pop(); continue

            """ Recursive Call and Value Updating """
            val_list = minimax(board, n_depth, False, alpha, beta, nhr,
                               opportunities, move_tuple[2], real_depth + 1, ndiff)
            ...
            if val_list[0] > best_list[0] or (val_list[0] == best_list[0] and len(val_list) >= len(best_list)):
                ...
                best_list = val_list
                best_list.append(board.uci(move_tuple[0]))
            ...
            """ Alpha Beta Pruning """
            if beta <= alpha:
                add_to_killers_and_history(move_tuple[0], real_depth)
                break
    ...
    # minimizing
        ...
        for move_tuple in ordered_moves:
            ...
            # Recursive Call and Value Updating
            val_list = minimax(board, n_depth, True, alpha, beta, nhr,
                               opportunities, move_tuple[2], real_depth + 1, ndiff)
            ...
            if val_list[0] < best_list[0] or (val_list[0] == best_list[0] and len(val_list) >= len(best_list)):
                # if real_depth == 0: print(f"{val_list[0]} <= {best_list[0]}")
                best_list = val_list
                best_list.append(board.uci(move_tuple[0]))  # append the move history
                if real_depth == 0: stats.top_moves.append(val_list)
