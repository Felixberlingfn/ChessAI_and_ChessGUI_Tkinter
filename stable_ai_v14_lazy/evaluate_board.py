...
def get_position_score(board) -> float:
    ...
    for square in good_queen_positions_white:
        piece = board.piece_at(square)
        if piece and piece.color == chess.WHITE and piece.piece_type == chess.QUEEN:
            score += GOOD_POS_BONUS
    ...
    for square in good_pawn_positions_black:
        piece = board.piece_at(square)
        if piece and piece.color == chess.BLACK and piece.piece_type == chess.PAWN:  # had forgotten piece.piece_type == chess.PAWN
            score -= GOOD_POS_BONUS
    bad_pawn_positions_black = [chess.D7, chess.E7]
    for square in bad_pawn_positions_black:
        piece = board.piece_at(square)
    ...
    for square in good_queen_positions_black:
        piece = board.piece_at(square)
        if piece and piece.color == chess.BLACK and piece.piece_type == chess.QUEEN:
            score -= GOOD_POS_BONUS
