...
def evaluate_board(board, horizon_risk=0.0, opportunities=0, material=None, real_depth=1, lost_castling=False) -> list:
    """
...
def get_position_score(board) -> float:
    ...
    """maybe I could check if I still have castling rights. I can reward still having
    castling rights as well as reward having castled. that way loosing the right to castle
    is punished. I just shouldn't punish loosing castling rights for castling.
    I could also do this in order moves at all king moves, check if the move ist a castling move,
    if not check if the move causes loosing castling rights and if so make the opportunity score worse"""
    ...
    good_rook_positions_white = [chess.E1, chess.D2, chess.E2, chess.A4, chess.H4]  # chess.C1, chess.D1,
    ...
    good_rook_positions_black = [chess.E8, chess.D7, chess.E7, chess.A5, chess.H5] # chess.D8,
    ...
    good_king_positions_black = [chess.G8, chess.B8]  # Typical castling squares for black king
