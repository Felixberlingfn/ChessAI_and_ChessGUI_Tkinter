...
def ai_0(board=None, time_limit=30) -> object:
    print(f"---------------{chess.COLOR_NAMES[int(board.turn)]}-AI V12_0 ----------------------------")
    # global end_time
    # end_time = time.time() + time_limit

    start_time = time.time()

    globals.time_limit = start_time + 30
    ...
    """at the start of the game (0 for white and 1 for black)"""
    if board.ply() == 0 or board.ply() == 1:
        print("resetting history and killer moves")
        reset_history_min()
        reset_history_max()
    ...
    init_material_balance = material_values_1 + material_values_2

    use_depth = INIT_DEPTH
    """if material_values_1 < 10 and material_values_2 > - 10:
        use_depth += 1"""
...
def print_results_and_stats(board, move_object, execution_time):

    move_id = len(board.move_stack)
    stats.printf(board.turn, move_id, execution_time)
