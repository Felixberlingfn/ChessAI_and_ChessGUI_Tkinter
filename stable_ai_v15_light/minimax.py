...
def minimax(board, depth, max_player, alpha=float('-inf'), beta=float('inf'), horizon_risk=0.0,
    ...
    if depth == 0 or board.is_game_over() or real_depth == CHECK_X_LIMITER:  # or time.time() > end_time:
        if last_move_type == CALM:
            return evaluate_board(board, horizon_risk, 0, material, real_depth, lost_castling)
        else:
            return quiescence.search(board, 2, max_player, last_move_type, last_victim_value, horizon_risk, op,
                                     material, real_depth, lost_castling, alpha, beta)
