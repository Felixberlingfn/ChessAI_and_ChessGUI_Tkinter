...
from . import stats, piece_sq_pesto, config
...
def ai_0(board=None, time_limit=30) -> object:
    print(f"-----------{chess.COLOR_NAMES[int(board.turn)]}-AI {config.AI_NAME} ----------------------------")
    # global end_time
    ...
    if board.ply() == 0 or board.ply() == 1:
        print("first move, resetting statistics and tables")
        reset_history_min()
        reset_history_max()
